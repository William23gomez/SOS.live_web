...
export class Register {
  ...
  async onSubmit() {
    ...
    const nitDv = this.formData.nitDv.trim();
    const password = this.formData.password;
    const { terms } = this.formData;

    if (!nombre || !email || !telefono || !nit || !nitDv || !password) {
    ...
    if (!/^\d$/.test(nitDv)) {
      this.showFeedback('El digito de verificaci\u00f3n del NIT debe ser un solo n\u00famero.', 'error');
      return;
    }
