...
export class Login implements OnInit {
  ...
  async onSubmit() {
    ...
    try {
      ...
      const message = this.resolveLoginErrorMessage(error, isAdminAccess);
      this.ngZone.run(() => {
        this.showFeedback(message, 'error');
      });
    } finally {
  ...
  async onResetPassword(event?: Event) {
    ...
    try {
      await this.authService.consultarEstadoVerificacionCorreo(email);
  ...
  private resolveLoginErrorMessage(error: unknown, isAdminAccess: boolean) {
    const errorCode =
