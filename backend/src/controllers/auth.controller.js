...
const validateUpdateProfileBody = ({ nombre, telefono, nit, direccion, plan }) => {
  if ([nombre, telefono, nit, direccion, plan].some(isBlank)) {
    const error = new Error('Completa todos los campos del perfil de empresa antes de guardar');
    error.statusCode = 400;
    throw error;
  }

  if (!/^\d+$/.test(String(telefono).trim()) || !/^\d+$/.test(String(nit).trim())) {
    const error = new Error('Telefono y NIT deben contener solo numeros');
    error.statusCode = 400;
    throw error;
  }
